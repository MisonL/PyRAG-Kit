...
def edit_retrieval_params() -> bool:
    """编辑检索相关参数。"""
    while True:
    ...
    return False # 检索参数的更改不需要重载任何模型

def edit_model_params() -> bool:
    """编辑模型相关参数。"""
    llm_changed = False
    while True:
        ...
        if choice == "llm":
            ...
            if new_rerank:
                ...
                console.print(f"[green]Rerank配置已切换为: {new_rerank}[/green]")
                # Rerank模型是按需加载的，所以不需要标记状态

    return llm_changed

def launch_config_editor() -> Tuple[bool, Dict[str, Any]]:
    """
    启动交互式配置编辑器。
    ...
    while True:
        ...
        if "1." in choice:
            # 检索参数的更改不会触发重载
            edit_retrieval_params()
        elif "2." in choice:
            # 如果LLM发生变化，标记需要重载
            if edit_model_params():
