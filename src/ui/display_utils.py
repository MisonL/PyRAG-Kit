...
def display_chat_config(console: Console, chat_config: dict[str, Any]):
    ...
    # --- 聊天配置 ---
    chat_table = Table(
        title="[bold green]聊天机器人配置[/bold green]", show_header=False, box=None, padding=(0, 1)
    )
    ...
    chat_table.add_row(
        "混合搜索权重 (向量/关键词):",
        f"{chat_config['vector_weight']} / {chat_config['keyword_weight']}",
    )
    ...
    chat_table.add_row(
        "Rerank重排:",
        "[bold green]启用[/bold green]"
        if chat_config["rerank_enabled"]
        else "[bold red]禁用[/bold red]",
    )
    if chat_config["rerank_enabled"]:
        ...
        chat_table.add_row(
            "激活的Rerank模型:",
            f"{active_rerank} ({rerank_model_details.provider}: {rerank_model_details.model_name})",
        )
    ...
    chat_table.add_row(
        "激活的LLM:", f"{active_llm} ({llm_model_details.provider}: {llm_model_details.model_name})"
    )
    ...
    api_table = Table(
        title="[bold green]API密钥与URL配置[/bold green]",
        show_header=False,
        box=None,
        padding=(0, 1),
    )
    ...
    if chat_config["rerank_enabled"]:
        active_providers.add(
            chat_config["rerank_configurations"][
                chat_config["active_rerank_configuration"]
            ].provider
        )
    ...
    console.print(
        Panel(
            Group(chat_table, api_table),
            title="[bold yellow]当前聊天会话配置[/bold yellow]",
            border_style="blue",
            width=CONSOLE_WIDTH,
        )
    )
