...
- `uv run python scripts/embed_knowledge_base.py`：重建知识库向量化缓存。
...
严禁提交密钥或本地覆盖配置。请复制 `config.ini.example` 或使用 `.env` 文件处理私密数值。将 `data/` 目录和生成的日志视为本地产物。若在 `dify/` 子目录下工作，请先阅读该目录下的 `AGENTS.md`，因为子目录指令优先于此根目录指南。
