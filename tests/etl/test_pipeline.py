...
def test_full_etl_pipeline_for_markdown(mock_markdown_document):
    ...
    """
    # 备份并临时修改全局配置以适应测试场景
    original_chunk_size = settings.kb_chunk_size
    original_chunk_overlap = settings.kb_chunk_overlap
    settings.kb_chunk_size = 100
    settings.kb_chunk_overlap = 20
    ...
    try:
        ...
        for doc in processed_docs:
            assert "  " not in doc.content.replace("`多余空格`", ""), "不应存在连续的两个空格"
        ...
        # 恢复原始配置，避免影响其他测试
        settings.kb_chunk_size = original_chunk_size
        settings.kb_chunk_overlap = original_chunk_overlap

def test_markdown_extractor(mock_markdown_document):
    """单独测试 MarkdownExtractor 的功能"""
    extractor = MarkdownExtractor()
    extracted_doc = extractor.extract(mock_markdown_document)
    # Markdown 提取器应该保留原始内容
    assert extracted_doc.content == SAMPLE_MARKDOWN
...
def test_basic_cleaner():
    ...
    assert cleaned_doc.content == "你好 世界 \n\n 再见."
...
def test_recursive_text_splitter():
    ...
    original_chunk_size = settings.kb_chunk_size
    original_chunk_overlap = settings.kb_chunk_overlap
    original_separators = settings.kb_splitter_separators
    settings.kb_chunk_size = 80
    settings.kb_chunk_overlap = 15
    settings.kb_splitter_separators = ["\n\n", "\n", " ", ""]

    try:
        splitter = RecursiveTextSplitter()
        # 使用一个更长的文本来测试分割和重叠
        long_text = ("这是一个非常长的句子，用于测试递归文本分割器的能力，确保它能在设定的块大小内进行切分。\n\n"
                     "这是第二段，它与第一段通过双换行符分隔，这应该是最优先的分割点。")
        ...
        # 验证第一部分的内容
        assert split_docs[0].content == "这是一个非常长的句子，用于测试递归文本分割器的能力，确保它能在设定的块大小内进行切分。"
        # 验证第二部分的内容
        assert split_docs[1].content == "这是第二段，它与第一段通过双换行符分隔，这应该是最优先的分割点。"
        # 验证元数据
        ...
        settings.kb_chunk_size = original_chunk_size
        settings.kb_chunk_overlap = original_chunk_overlap
        settings.kb_splitter_separators = original_separators
